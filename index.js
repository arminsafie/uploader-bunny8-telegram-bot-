...
const LARAVEL_API_URL = "http://192.168.0.148:8000/store-user";

// Function to send user data to Laravel
async function sendUserDataToLaravel(user) {
  try {
    ...
    console.error("Error sending user data to Laravel:", error.message);
  }
}
...
bot.onText(/\/start/, async (msg) => {
  ...
  await sendUserDataToLaravel({
    message_id: user.id.toString(),
    first_name: user.first_name,
    username: user.username,
  });
...
bot.on("callback_query", async (query) => {
  ...
  switch (action) {
    ...
    case "start":
      bot.sendMessage(chatId, "bot is Already start");
      break;
